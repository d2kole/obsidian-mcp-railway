...
| `OAUTH_ACCESS_TOKEN_TTL_SEC`      | no (default `86400`)                                | Access token lifetime in seconds. There is **no refresh-token grant**, so when a token expires the MCP client must re-run the full OAuth flow — which re-prompts for the PAT. To avoid re-authenticating roughly daily, set a longer window: `7776000` (90 days, recommended for single-user) or `31536000` (1 year). Revoke a leaked long-lived token with `POST /admin/tokens/<jti>/revoke`, or rotate `OAUTH_CLIENT_SECRET` + `SESSION_ENCRYPTION_KEY` to invalidate all of them. The TTL is baked into each token at issue time, so a change only affects tokens minted after the next deploy. |
...
| `VAULT_GIT_TIMEOUT_MS`            | no (default `60000`)                                | Hard-kills any git subprocess (clone/pull/push/commit) that runs longer than this via simple-git's `timeout.block`. Prevents a hung remote from leaking an un-reapable process — see 2026-09-16 pid-exhaustion incident. Non-positive values fall back to the default.                                                                                                                                                                                                                                                                                                                             |
| `VAULT_SYNC_MIN_INTERVAL_MS`      | no (default `5000`)                                 | Throttles `sync()` so concurrent tool calls don't each spawn their own `git pull --rebase`; callers within the window (or already mid-pull) join the in-flight pull instead. **Previously defaulted to `0` (no throttle) — this was the root cause of the 2026-09-16 outage** (every tool call pulling with no cap eventually exhausted container pids). Explicit `0` is honored if you deliberately want no throttle; anything else invalid falls back to the default.                                                                                                                            |
| `VAULT_HEALTH_FETCH_TIMEOUT_MS`   | no (default `5000`)                                 | Separate, shorter `timeout.block` used only by the `/api/healthz` dry-run fetch, so a slow upstream can't block Railway's healthcheck. Unlike the pre-fix implementation (a `Promise.race` that abandoned the promise but never killed the subprocess), this timeout force-kills the actual git process.                                                                                                                                                                                                                                                                                           |
...
1. **Inline marker** (preferred): append `secret-scan: allow` as a
   ...
   ```ts
   const input =
     "https://x-access-token:ghp_AAAAAAAAAAAA@github.com/foo/bar.git"; // secret-scan: allow (test fixture)
   ```
