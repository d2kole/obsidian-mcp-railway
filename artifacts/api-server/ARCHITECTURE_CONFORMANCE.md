...
| `VAULT_SYNC_MIN_INTERVAL_MS` throttle | **PRESENT** | `src/lib/config.ts` (`syncMinIntervalMs`), consumed in `src/vault/service.ts` `sync()`                                       | Optional, default 5000ms (was 0/no-throttle until the 2026-09-16 pid-exhaustion fix); concurrent/in-flight callers dedupe via `syncPromise` regardless of throttle |
