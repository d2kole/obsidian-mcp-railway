...
**On every read tool call,** `VaultService.sync()` runs `git pull --ff-only` so Claude always sees the latest commit pushed by Desktop. Throttled by default (`VAULT_SYNC_MIN_INTERVAL_MS`, default 5000ms); concurrent callers within the window, or mid-pull, join the in-flight pull instead of spawning their own.
...
- **Healthcheck is binary.** A single failed sub-check returns 500 and Railway will restart the container. Network blips on `git fetch --dry-run` will cause restarts; that's by design (the fetch subprocess is hard-capped and force-killed at `VAULT_HEALTH_FETCH_TIMEOUT_MS`, default 5000 ms — see 2026-09-16 pid-exhaustion incident for why this is a real process kill now, not just an abandoned promise).
